    ...
    static connexionBdDev() {
        return __awaiter(this, void 0, void 0, function* () {
            const sequelize = new sequelize_1.Sequelize({
                database: process_1.env.BD_NAME,
                host: process_1.env.HOST,
                dialect: "mysql",
                username: process_1.env.BD_USER,
                password: process_1.env.BD_PWD,
                port: Number.parseInt(process_1.env.PORT_DEV),
            });
            try {
                //CONNEXION A LA BD DISTANTE CLOUDCLUSTERS
                yield sequelize.authenticate();
                console.log("[mode pro] Connexion à la BD réussit.");
                //Init Models
                ...
                if (dataBoolean) {
                    sequelize
                        ...
                        insertDataInitDb_1.InsertDataInitDb.initialiseurDefaultData(dataSequelize)
                            ...
                            console.log("[mode pro] La BD a été bien connectée.");
                        })
                            .catch((error) => {
                            console.log("[mode pro] erreur init " + error);
                        });
                    })
                        .catch((error) => {
                        console.log("[mode pro] Une erreur c'est produite l'or de la synchronisation de la BD : " +
                            error);
                    });
                }
            }
            catch (error) {
                console.log("[mode pro] Une erreu s'est produite l'or de la connexion à la BD :\n" +
                    error);
            }
        });
    }
