...
import routesHelpers from "../../routes/routes.helper";
dotenv.config();
//
...
export default async (req: Request, res: Response) => {
  try {
    ...
    const dataCompteBloquer = await getModels(
      NameModelsListe.cmpBloquer
    ).findOne({
      where: { email },
    });
    if (dataCompteBloquer) {
      return res.status(403).json({
        message:
          "Désoler votre compte est bloqué! Merci de contacter votre administrateur.",
      });
    } else {
      //
